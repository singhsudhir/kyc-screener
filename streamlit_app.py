...
# ── Inject Streamlit secrets into os.environ BEFORE LangChain/Google imports ──
def _inject_secrets() -> None:
    for _k in ["GEMINI_API_KEY", "TAVILY_API_KEY", "OPENSANCTIONS_API_KEY", "GROQ_API_KEY"]:
        try:
            _v = st.secrets.get(_k)  # type: ignore[arg-type]
...
DEMO_SCENARIOS: list[tuple[str, str, str]] = [
    ("clean_corp",     "Clean Corp BV",      "🟢  GREEN  · Score 12"),
    ("global_trading", "Global Trading Ltd", "🟡  AMBER  · Score 58"),
    ("shadow_finance", "Shadow Finance SA",  "🔴  RED    · Score 91"),
]
...
# ─────────────────────────────────────────────────────────────────────────────
# Sidebar + mode helpers (cloud-specific)
# ─────────────────────────────────────────────────────────────────────────────
...
def _render_demo_banner_cloud(live_mode: bool) -> None:
    if live_mode:
        st.markdown(
            '<div style="display:flex;align-items:center;gap:0.6rem;'
            ...
            '<span style="font-size:0.95rem;flex-shrink:0;">🟢</span>'
            '<div>'
            '<span style="font-size:0.78rem;font-weight:700;color:#22C55E;">Live Mode</span>'
            '<span style="font-size:0.78rem;color:#8B949E;"> — real AI agents, live data sources, API calls are charged.</span>'
            '</div>'
            '</div>',
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            ...
            '<span style="font-size:0.95rem;flex-shrink:0;">🎯</span>'
            '<div>'
            '<span style="font-size:0.78rem;font-weight:700;color:#388BFD;">Viewing demo screening</span>'
            '<span style="font-size:0.78rem;color:#8B949E;"> · Switch to Live Mode via the sidebar to screen any company.</span>'
            '</div>'
            '</div>',
            unsafe_allow_html=True,
        )
...
def _render_scenario_cards() -> str | None:
    """Render 3 demo scenario cards. Returns the key of the clicked scenario, or None."""
    st.markdown(
        '<p style="font-size:0.72rem;color:#8B949E;margin:0 0 0.75rem;line-height:1.6;">'
        'Select a pre-built screening scenario to explore the full report:</p>',
        unsafe_allow_html=True,
    )
    cols = st.columns(3)
    for col, (key, label, score_str) in zip(cols, DEMO_SCENARIOS):
        report    = MOCK_REPORTS[key]
        level     = report["risk_rating"]["level"]
        score     = report["risk_rating"]["score"]
        cfg       = RISK_CFG[level]
        jur       = report["entity"]["jurisdiction"]
        industry  = report["entity"].get("industry", "")
        with col:
            clicked = st.button(
                f"{cfg['icon']}  {label}\n{jur}  ·  {industry[:20]}",
                key=f"demo_scenario_{key}",
                use_container_width=True,
                help=f"{cfg['label']}  ·  Score {score}/100",
            )
            if clicked:
                return key
    return None
...
# ─────────────────────────────────────────────────────────────────────────────
# Animated stepper while demo runs (no API call)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Inline LangGraph screening (Live Mode — no HTTP backend required)
# ─────────────────────────────────────────────────────────────────────────────

def _run_screening_inline(payload: dict) -> None:
    """Run LangGraph screening inline — works without a FastAPI backend."""
    # Lazy import so secrets are already in os.environ by the time LangChain initialises
    ...
    def _call() -> None:
        try:
            entity = CompanyEntity(**payload)
            raw    = asyncio.run(
                kyc_graph.ainvoke({"entity": entity, "flags": [], "errors": []})
            )
            result["report"] = raw["report"].model_dump(mode="json")
    ...
    st.session_state["screening_elapsed"] = elapsed
    st.session_state["last_report_live"]  = True
    history = st.session_state.get("history", [])
...
if live_mode:
    ...
    # ── Demo mode: show scenario cards ────────────────────────────────────────
    clicked_key = _render_scenario_cards()

    if clicked_key:
        st.session_state["demo_scenario_key"] = clicked_key
        mock = copy.deepcopy(MOCK_REPORTS[clicked_key])
        _run_demo_screening(mock)
    elif "last_report" in st.session_state and not st.session_state.get("last_report_live"):
        _render_report(st.session_state["last_report"])
    elif "demo_scenario_key" in st.session_state:
        mock = copy.deepcopy(MOCK_REPORTS[st.session_state["demo_scenario_key"]])
        _run_demo_screening(mock)
    else:
        # Auto-load first scenario on first visit
        st.session_state["demo_scenario_key"] = "clean_corp"
        mock = copy.deepcopy(MOCK_REPORTS["clean_corp"])
        _run_demo_screening(mock)
